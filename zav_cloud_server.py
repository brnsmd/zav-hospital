...
def telegram_webhook():
    ...
    try:
        ...
        if text and text.startswith("/external-patient"):
            try:
                # Parse: /external-patient Name, Age, Operation
                parts = text.replace("/external-patient", "").strip().split(",")

                if len(parts) < 3:
                    send_telegram_message(chat_id, "❌ Invalid format.\nUse: /external-patient Name, Age, Operation, [Details]")
                    return jsonify({"ok": True}), 200
                ...
                if db:
                    ...
                    """, (f"EX{chat_id}{int(datetime.now().timestamp())}", patient_name, "pending_external", "telegram"))
                    logger.info(f"✅ External patient stored: {patient_name}")

                # Send confirmation
                confirmation = f"""✅ <b>Patient Request Recorded!</b>

<b>Patient:</b> {patient_name}, Age {patient_age}
<b>Operation:</b> {operation}
<b>Details:</b> {details if details else 'None'}

📋 Status: Added to pending review
⏰ Next Review: 5 AM daily sync or when hospital staff opens Claude
"""
        ...
        # Handle /start command
            ...
            welcome = """🏥 <b>Zav Hospital Management Bot</b>

<b>For Doctors:</b> Submit external patient requests
...
→ Submit new external patient request

<b>/start</b> - Welcome message
<b>/help</b> - This message
<b>/status</b> - System status

All requests are reviewed daily at 5 AM and by hospital staff in Claude.
"""
